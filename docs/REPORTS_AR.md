...
سعر الشراء اختياري، ويُحفظ مشفرًا في قاعدة البيانات لكل منتج ولكل variant. لا يظهر في استجابات المنتجات العامة أو استجابات الـ API العادية، ولا يستطيع إدخاله أو تعديله أو قراءته إلا المستخدم الذي يحمل دور `owner`.

```http
GET /api/v1/products/{productId}/purchase-price
GET /api/v1/products/{productId}/variants/{variantId}/purchase-price
GET /api/v1/reports/profitability?from=2026-09-01&to=2026-09-14
```

تقرير الربحية محمي للمالك فقط، ويعرض الإيراد، تكلفة الشراء، الربح الإجمالي، ونسبة اكتمال بيانات التكلفة. لا يعتبر التقرير ربحًا محاسبيًا نهائيًا لأنه لا يخصم عمولات الدفع، مصاريف الشحن، الضرائب، أو تكلفة المرتجعات ما لم تُسجل هذه البنود في نماذج مالية مستقلة.
