...
```http
GET /api/v1/admin/shipments?q=TRK-123&status=in_transit&carrier=Bosta&from=2026-09-01&to=2026-09-14
```

تدعم البحث برقم التتبع أو رقم الطلب أو بيانات العميل، والفلترة بالحالة وشركة الشحن والفترة الزمنية.

### تفاصيل الشحنة

```http
GET /api/v1/admin/shipments/{shipment}
```

تعرض الطلب والعميل وشركة الشحن، وأحداث الشحنة، وعمليات المزود، وعدد المحاولات وآخر خطأ. لا يتم إرجاع payloads الخاصة بمزود الشحن.

### الشحنات المتأخرة أو الفاشلة

```http
GET /api/v1/admin/shipments/exceptions?carrier=Bosta&older_than_hours=48
```
