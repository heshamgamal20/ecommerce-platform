...
{
    public function index(ProductReviewRequest $request, int $product, ManageProductReviews $useCase): JsonResponse { return response()->json(['data' => $useCase->approved($product, (int) $request->validated('per_page', 25)), 'meta' => $useCase->summary($product)]); }
    ...
    public function adminIndex(ProductReviewRequest $request, ManageProductReviews $useCase): JsonResponse { return response()->json(['data' => $useCase->all($request->user(), (int) $request->validated('per_page', 25))]); }
