...
{
    public function approvedForProduct(int $productId, int $perPage = 25): object { Product::query()->findOrFail($productId); return ProductReview::query()->with('user:id,name')->where('product_id', $productId)->where('status', 'approved')->latest()->paginate(min(max($perPage, 1), 100)); }
    ...
    public function all(int $perPage = 25): object { return ProductReview::query()->with(['product:id,name', 'user:id,name'])->latest()->paginate(min(max($perPage, 1), 100)); }
